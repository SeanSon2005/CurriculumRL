...
class Action_Model(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.emb = nn.Embedding(1,MODEL_PARAMETERS['dim'])
    ...
    def forward(self, obs):
        # Get only the items that are in view of the robot
        items = np.copy(obs['location'][:obs['num_items']])
        input = self.emb(items)

        transformer_encoding = self.transformer(input)
...
class DeepQControl:
    ...
    def __init__(self,observation,device, num_steps):
        ...
        self.policy_net = Action_Model(n_observations, n_actions).to(device)
        self.target_net = Action_Model(n_observations, n_actions).to(device)
